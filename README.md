...
2. Extract it to a versioned directory such as `C:\BroadcastRouter\1.6.2`.
...
Live-video supervision also detects a stale decoder that keeps FFmpeg output progress alive by repeating almost every frame. After the configurable frozen-input timeout, only that owned route process is stopped and retried; the saved assignment and output-port configuration remain intact. Audio-led sources are excluded because their continuous black video is intentional. Ambiguous H.264/AAC inputs receive a bounded, rate-limited extended keyframe-acquisition probe before audio-led mode is committed, so a long GOP is not mistaken for an absent picture. Fatal RTSP control-session desynchronization, confirmed post-startup DeckLink queue starvation, repeated AAC decoder corruption, and repeated invalid audio timestamps recreate only the affected owned route while preserving its saved output reservation. One isolated damaged packet or queue warning is tolerated; repeated evidence inside a bounded window triggers exact-PID recovery. Per-session diagnostic counters record the classified evidence without retaining media URLs or credentials. During recovery, the generated fallback is identified separately and fully reaped before replacement live playout starts, so its frames cannot be reported as recovered source video or block the new source owner. **Refresh discovery** on Sources is a source-only operation and displays the backend-confirmed completion time and inventory count without rescanning DeckLink hardware.

The routing worker publishes an internal heartbeat before and after process supervision, discovery/probing, hardware validation, route reconciliation, and standby reconciliation. The independent fast input and publisher supervisors have separate heartbeats and restart themselves after an unexpected exception; normal reconciliation still consumes a captured media failure if fast supervision is temporarily restarting. `/health` reads a once-per-minute background SQLite integrity snapshot and becomes `degraded` if the database, coordinator, or either critical auxiliary loop stops making progress. Monitoring requests therefore remain non-blocking and cannot contend with live database traffic. The independent watchdog records the stalled component and terminates the host only after its bounded silence threshold so the configured Windows Service recovery action can restart it; the process Job Object removes only BroadcastRouter-owned media children. This protects against a responsive web interface masking a stalled routing engine.
...
Discovery reuses confirmed standard-video metadata only while that source's exact owned live FFmpeg process is running and reporting fresh frames. This removes redundant FFprobe/keyframe scans from healthy routed video while new, offline, unassigned, audio-led, fallback, stale, or unhealthy sources remain fully probe-gated.
...
```powershell
.\scripts\Publish-Release.ps1 -Version 1.6.2
```
