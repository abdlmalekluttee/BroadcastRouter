...
7. Disable any previous interactive-logon BroadcastRouter task. From an elevated PowerShell window, obtain a credential for the same dedicated Windows account and run:
   ...
   Keep those recovery actions enabled. Version 1.5.4 treats a coordinator that makes no progress for two minutes as a fatal service fault, records the blocked stage, and exits so SCM can restore discovery/routing automatically. Monitor `/health`; `degraded` includes a stale routing coordinator as well as a failed database integrity check. From version 1.5.24, database integrity is refreshed once per minute in the background, so frequent health polling is non-blocking and does not contend with live SQLite traffic.
   Version 1.5.25 also avoids redundant FFprobe work for a confirmed standard-video source while its exact owned live process is advancing. New, offline, unassigned, audio-led, fallback, stale, and unhealthy sources remain fully probe-gated.
   Version 1.5.28 reduces continuous standby CPU/thread pressure without GPU acceleration. Synthetic standby and audio-led black inputs no longer repeat no-op raster/rate conversion, unchanged logos are decoded at 1 fps and held by the overlay graph, and each per-port standby complex graph uses one worker. Output raster, progressive/interlaced cadence, top-field-first metadata, `uyvy422`, silent PCM audio, and exclusive DeckLink ownership are unchanged. Physically verify every configured standby and compare host CPU before accepting the upgrade.
   Version 1.5.5 isolates DeckLink reference polling in a five-second helper process. Version 1.5.22 closes the remaining in-host identity-discovery path, preserves the last confirmed port inventory during bounded retries, and separates local FFmpeg failure supervision from Wowza REST latency. Version 1.5.23 keeps an isolated DeckLink identity-helper timeout on the normal five-minute hardware-validation cadence instead of rerunning the complete validation every 30 seconds. Version 1.5.6 adds bounded long-GOP confirmation and route-scoped RTSP protocol recovery. Version 1.5.7 serializes fallback-to-live ownership and isolates supervision faults per source. Version 1.5.8 also handles a live PID disappearing between supervision and saved-route reconciliation. Version 1.5.9 adds a fast fatal-session supervision path and DeckLink media-starvation recovery for rapid Wowza resets. Version 1.5.10 reacts to post-startup video or audio starvation and executes due live retries on that same fast path. Version 1.5.11 adds a 1000 ms default RTSP read timeout and permits saved routes to recover from their known URI while an overlapping discovery sample is stale. Versions 1.5.14 through 1.5.17 add authoritative rapid publisher-loss/return recovery and retained media readiness; version 1.5.18 uses a bounded one-second connected-publisher recovery cadence, version 1.5.19 keeps saved waiting-for-stream routes in that fast recovery path, and version 1.5.20 also recovers a connected starting/running route after its short-lived live owner exits. Version 1.5.26 debounces isolated media anomalies, recovers on repeated AAC/timestamp/starvation evidence, and makes both fast supervision loops self-restarting and health-visible. Version 1.5.27 closes the process-publication/disposal snapshot race observed during initial output reconstruction.
