...
Version 1.5.28 requires no settings or database migration and enables no GPU processing. Existing port, standby, logo, route, and preset configuration remains compatible. The generated standby/audio-led video path removes redundant CPU transforms, decodes a static logo at 1 fps, and bounds the per-port complex filter graph to one worker. After deployment, physically verify progressive and 1080i50 standby picture, clock/logo placement, digital silence, field order, and standby-to-live recovery on every output, then compare total host and child-FFmpeg CPU against the previous version.
